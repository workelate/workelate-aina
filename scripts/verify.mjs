...
// Footer parity is PER ZONE (2026-09-12): the umbrella landing, the product
// site under /brain and the studio under /studio each carry their own footer,
// generated by scripts/genchrome.mjs. Parity still holds inside a zone.
import { zoneOf } from "./genchrome.mjs";
const footerBaseline = {}, footerBaselinePage = {};
...
for (const route of PAGES) {
  ...
  const facts = await page.evaluate((banned) => {
    ...
    // Is this page on the brand system? Measured, not asserted: the ground the
    // body actually paints, and the family the body actually renders in.
    const bodyStyle = getComputedStyle(document.body);
    const ground = bodyStyle.backgroundColor;
    const family = bodyStyle.fontFamily;
  ...
  if (facts.footerLinks === null) {
    ...
    if (!footerBaseline[zone]) { footerBaseline[zone] = facts.footerLinks; footerBaselinePage[zone] = route; }
    else {
      const a = new Set(footerBaseline[zone]), b = new Set(facts.footerLinks);
      const missing = [...a].filter(x => !b.has(x)), extra = [...b].filter(x => !a.has(x));
      assert("footer parity", missing.length === 0 && extra.length === 0, route,
        `vs ${footerBaselinePage[zone]}: missing ${JSON.stringify(missing)} extra ${JSON.stringify(extra)}`);
    }
  }
...
// it, ask a question, and require an answer with zero console errors.
{
  ...
  page.on("pageerror", e => errors.push("pageerror: " + e.message));
  page.on("console", m => m.type() === "error" && errors.push(m.text()));
  // The assistant lives on the STUDIO home since the 2026-09-12 restructure;
  // the umbrella landing at "/" is two doors and carries no chat input.
  const ASSISTANT_ROUTE = "/studio";
  await page.goto(URL + ASSISTANT_ROUTE, { waitUntil: "networkidle", timeout: 30000 });
  ...
  assert("assistant answers a question", probe.found && probe.grew, ASSISTANT_ROUTE,
    probe.found ? "asked a question, page text did not grow (no answer rendered)" : `no assistant input found on ${ASSISTANT_ROUTE}`);
  assert("zero console errors during assistant use", errors.length === 0, ASSISTANT_ROUTE, errors.join(" | "));
  await page.close();
}
