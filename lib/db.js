  ...
  try {
    ...
    db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS chat_leads (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        contact TEXT, kind TEXT, industry TEXT, transcript TEXT, path TEXT, ua TEXT, ts TEXT
      );
    `);
...
// A lead captured mid-conversation by the on-page assistant. `contact` is an
// email or a phone; `transcript` is the chat so far, so the founders can read
// the intent, not just the address. Returns the new id, or null when no store.
export async function insertChatLead({ contact, kind, industry, transcript, path, ua, ts }) {
  const s = await store();
  if (!s.db) return null;
  const { lastInsertRowid } = s.db.prepare(
    "INSERT INTO chat_leads (contact, kind, industry, transcript, path, ua, ts) VALUES (?,?,?,?,?,?,?)"
  ).run(contact, kind, industry || "", String(transcript).slice(0, 8000), path || "", String(ua || "").slice(0, 300), ts);
  return Number(lastInsertRowid);
}
